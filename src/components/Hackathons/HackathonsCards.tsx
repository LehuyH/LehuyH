...
export function HackathonsCards() {
    const [activeHackathon, setActiveHackathon] = useState<HackathonInfo | null>(null)

    return (<>
        <p className="pb-6 pt-2 px-8">
            So far, I&apos;ve participated in {hackathons.length}+ hackathons with my friends!
        </p>
        <div className="flex flex-col md:flex-row max-w-4xl px-8 mx-auto gap-4 justify-center items-center">
            ...
            </section>
            {
                ...
                <article className="space-y-4">
                    <aside className="flex gap-4 justify-center items-center">
                        <button aria-label="Back" onClick={() => setActiveHackathon(null)} className="text-sky-600 hover:underline font-serif">
                            ←
                        </button>
                        <h1 className="text-3xl font-serif tracking-wider font-light">{activeHackathon.name}</h1>
                    </aside>
                    <p className="text-lg leading-relaxed text-left">{activeHackathon.description}</p>
                    <a href={activeHackathon.link} target="_blank" className="text-sky-600 hover:underline font-serif block">
                        View Project
                    </a>
        ...
        {
            !activeHackathon &&
            <p className="text-center text-lg pt-12">
