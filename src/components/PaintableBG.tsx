...
export function PaintableBG({ className, containerRef }: { className?: string, containerRef: React.RefObject<HTMLElement> }) {
    const canvasRef = useRef<HTMLCanvasElement>(null)
    const [dims,setDims] = useState(null as {
        width: number,
        height: number
    } | null)
    ...
    return (
        dims &&
        <canvas width={dims.width} height={dims.height} ref={canvasRef} className={`${className}`}></canvas>
    )
}